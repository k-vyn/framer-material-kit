...
exports.create = (array) ->
	...
	field = new Layer borderRadius:m.utils.px(setup.borderRadius), backgroundColor:setup.backgroundColor, width:m.utils.px(setup.width), height:m.utils.px(setup.height)
	if setup.constraints
		field.constraints =
			setup.constraints
	field.active = false
	text = new m.Text style:"fieldText", superLayer:field, text:setup.text, fontSize:setup.fontSize, fontWeight:setup.fontWeight, color:setup.color
	...
	if setup.text == "" || setup.text == undefined
		placeholder = new m.Text style:"fieldPlaceholder", superLayer:field, text:setup.placeholderText, fontSize:setup.fontSize, fontWeight:setup.fontWeight, color:setup.placeholderColor
		if setup.textConstraints
	...
	field.on Events.TouchEnd, ->
		...
		if setup.input
			keyboard = new m.Keyboard animated:true, output:field, returnText:setup.returnText, returnColor:setup.returnColor
		...
		if field.cursor == undefined
			...
			cursor = new Layer width:m.utils.px(setup.cursor.width), height:m.utils.px(setup.cursor.height), superLayer:field, name:"cursor", backgroundColor:m.utils.color("blue"), borderRadius:m.utils.px(1)
			field.cursor = cursor
			cursor.constraints =
	...
	keypress = (key) ->
		...
		switch key.name
			...
			when "delete"
				...
				key.icon.states.switchInstant("on")
			when "space"
				key.backgroundColor = m.utils.color("light-key")
			else
				if m.device.name != "ipad"
					...
					key.animate
						properties:(backgroundColor:m.utils.color("light-key"))
						time:.2
	...
	document.addEventListener 'keydown', (e) ->
		if field.active
			...
			if e.keyCode == 8
				...
				field.value = m.utils.clean(newText)
